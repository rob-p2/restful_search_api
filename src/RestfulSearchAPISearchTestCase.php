...
class RestfulSearchAPISearchTestCase extends \DrupalWebTestCase {
  ...
  public function setUp() {
    ...
    for ($i = 1; $i <= static::NUM_ITEMS; ++$i) {
      ...
      \Drupal::moduleHandler()->invokeAll('entity_insert', [search_api_test_load($id), 'search_api_test']);
    }
  ...
  public function testSearch() {
    $handler = restful()->getResourceManager()->getPlugin('search:1.0');
    // Search all the items that contain the keyword title.
    $results = $handler->doGet('Title');
    $this->assertFalse(empty($results[0]), 'Some results were found.');
  }
